 ...
 *
 * Branch 1 is exercised incidentally through the dashboard happy path.
 * Branches 2 and 3 with falsy-field fallbacks (GroupsList.jsx lines
 * 27-28) are only reachable when the graph payload omits groups or
 * leaves `description` / `mail` off the records. The default mock
 * (`mock/graphApi.js`) always returns a populated set with both fields,
 * so without a controlled payload those branches stay at 0%.
 *
 * `mock/graphApi.js` honours a `MOCK_GROUPS_OVERRIDE` localStorage
 * key: write a JSON array there before the dashboard mounts and
 * `getAllGroups` returns it verbatim. The key follows the same pattern
 * as the existing `MOCK_LOGIN_FAIL` / `MOCK_INTERACTION_REQUIRED`
 * hooks in `mock/azureMsalBrowser.js`.
 */
...
describe('GroupsList — empty-state and falsy-field branches', () => {
  ...
  // Write a controlled groups payload before the Dashboard mounts so
  // getAllGroups reads the override on its first call. Without this
  // the dashboard hits the canned mock data and the empty-state /
  // falsy-field branches stay at 0%.
  ...
  it('renders the empty-state placeholder when Graph returns no groups', () => {
    ...
    // Cleanup: ensure subsequent tests in this run see the canned data.
    clearGroupsOverride();
  });
