 ...
 * /access-denied.
 *
 * Note on the previous shape: an earlier draft of this spec
 * intercepted `/me/memberOf` and `/api/admin-data`, but neither is
 * reachable from the e2e in mock mode (the Graph endpoint is
 * replaced by `mock/graphApi.js` which returns mock data without a
 * network call, and `getAdminData` is not invoked by any page in the
 * production UI). The tests below use the throwing endpoints that
 * the dashboard and experiments page actually call.
 */
...
describe('Genuine 4xx/5xx backend errors surface as user-visible toasts', () => {
  ...
  it('surfaces a 500 on /lab-experiments (GET) as a notyf error on the experiments page', () => {
    ...
    // The page itself remains mounted, and the experiments error
    // alert also surfaces — both are driven by the same throw.
